...
deseq_diffspec_taxmap <- function(ps2.tmp){
  ...
  ps2.tmp.tax@sam_data$source <- factor(ps2.tmp.tax@sam_data$source, levels=levels(ps2.tmp@sam_data$source))
...
ps2.deseq2.difftable <- rbind(ps2.deseq2.difftable, data.table(taxon_id=ps2dds.asso.worm.tax$taxon.id, treatment_2="substrate", treatment_1="host", log2FoldChange=ps2dds.asso.worm.tax$log2FoldChange, baseMean=ps2dds.asso.worm.tax$baseMean, padj=ps2dds.asso.worm.tax$padj))
...
if(nrow(ps2dds.alone.asso[padj<=0.05]) > 0){ # alone vs. asso has now differential abundany species
	ps2.deseq2.difftable <- rbind(ps2.deseq2.difftable, data.table(taxon_id=ps2dds.alone.asso$taxon.id, treatment_2="control", treatment_1="substrate", log2FoldChange=ps2dds.alone.asso$log2FoldChange, baseMean=ps2dds.alone.asso$baseMean, padj=ps2dds.alone.asso$padj))
}
...
ps2.deseq2.difftable <- rbind(ps2.deseq2.difftable, data.table(taxon_id=ps2dds.alone.worm$taxon.id, treatment_2="control", treatment_1="host", log2FoldChange=ps2dds.alone.worm$log2FoldChange, baseMean=ps2dds.alone.worm$baseMean, padj=ps2dds.alone.worm$padj))	
